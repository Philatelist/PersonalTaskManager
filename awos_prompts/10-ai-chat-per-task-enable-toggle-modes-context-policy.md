# AWOS Topic Prompt: AI Chat Per Task: Enable Toggle + Modes + Context Policy

This single file is intended to be used sequentially with AWOS standard commands:

- `/awos:spec  @awos_prompts/09-ai-chat-per-task-enable-toggle-modes-context-policy.md`
- `/awos:tech  @awos_prompts/09-ai-chat-per-task-enable-toggle-modes-context-policy.md`
- `/awos:tasks @awos_prompts/09-ai-chat-per-task-enable-toggle-modes-context-policy.md`

## Topic
AI Chat Per Task: Enable Toggle + Modes + Context Policy

## Scope (what this topic covers)
Per-task AI enable; exactly one chat thread per task; 4 modes; per-task context policy (all vs summary+last N); notes sharing policy; suggested changes format (proposal + apply).

## Out of scope (for this topic)
Advanced prompt engineering beyond modes; server backend.
