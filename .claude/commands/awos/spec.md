...
After successfully generating/updating spec outputs (e.g. `context/spec/**/functional-spec.md`):

1) Determine topic (for commit message)
- If called with: `/awos:spec @awos_prompts/<nn>-<topic>.md`
- Extract `<topic>` from the filename (remove leading `<nn>-` and trailing `.md`)
- Convert dashes to spaces
- Example: `01-core-task-model-subtasks-progress.md` → `core task model subtasks progress`
